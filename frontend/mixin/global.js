...
export default {
  data () {
    return {
      ...
      placeholderActionUrl: 'http://b2b.ku-min.com:3001',
      defaultAvatar: '/imgs/default-avatar.png',
      defaultLogo: '/imgs/dashboard-logo.png',
      smsDisabled: false
    }
  },
  created () {
  ...
  methods: {
    ...
    checkDuplicates () {
      let params = {}
      if (this.$auth.user.user_type === 1 && (this.user_type === 'requester' || this.user_type === 'manager')) {
        if (!this.form.company_id) {
          this.$message.error(this.$t('validation.company_required'))
          return
        }
        params = {
      ...
      this.$axios
        .post('/basic/duplicate-id-check', params).then((resp) => {
    ...
    async getTags () {
      ...
      this.tagOptions = resp.data.data
    },
    getLangName (langId) {
      return this.$t('common.' + this.workLangs.filter(lang => lang.id === langId)[0].name)
    },
    numberWithCommas (x) {

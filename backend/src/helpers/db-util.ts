...
export const sendPush = async(_parent: User | null, _admins: User[], _user: User, _trans_request: TransRequest | null, _request_detail: RequestDetail | null, _type: any, _is_read = Trigger.OFF) => {
    let _objects;
    if( _type == 1 && _trans_request != null &&_user.user_type == 3 && _trans_request.has_original_video == 'N' && _parent?.has_tc_service == 'Y') {
        ...
        for(let i = 0; i < _objects.length; i ++) {
            await noticeProc(_parent, _admins, _objects[i], _trans_request, null, 1, Trigger.OFF)
        }
    }
    ...
    || (_type == 1 && _trans_request != null && (_trans_request.has_original_video == 'Y' || _parent?.has_tc_service == 'N') && _user.user_type == 3) ) {
        ...
        for(let i = 0; i < _objects.length; i ++)
                for(let j = 0; j < _objects[i].length; j ++)
                    await noticeProc(_parent, _admins, _objects[i][j], _trans_request, _trans_request.details[i], 1, Trigger.OFF)
    }
        ...
        _objects = await query.getMany();
        for(let i = 0; i < _objects.length; i ++) {
            await noticeProc(_parent, _admins, _objects[i], null, _request_detail, 1, Trigger.OFF)
        }
    }
    else
        return;
...
type: 12-> 댓글 등록    16
*/
export const noticeProc = async(_parent: User | null, _admins: User[], _user: User, _trans_request: TransRequest | null, _request_detail: RequestDetail | null, _type: any, _is_read = Trigger.OFF) => {
    try {
        ...
        //TC알림 인 경우
            ...
            if(_type == 1 && _parent != null) {
                template_obj.predict_end_date = (await calc_working_time(_parent.screen_time_limit, _parent.end_time_settings, _trans_request.duration_minute, 4, _trans_request.is_urgent)) * 60 + _trans_request.create_date
            }
            template_obj.site_url = config.SITE_URL;
            insertNoticeUser(_user, _notice_ret)

            webPush([_user.player_id], _user.system_lang, convertTranslateMsg(lang[_user.system_lang]["web_push"]["tc_a01"], {title: _trans_request.title, org_lang: _trans_request.original_language.name}))
                ...
                if(_user.country_code == '+82' && _user.system_lang == 'KO') {
                    ...
                    if(_type == 1) {
                        let msg = convertTranslateMsg(lang[_user.system_lang]["sms"]["tc_a01"], {
                            title: _trans_request.title,
                            org_lang: _trans_request.original_language.name
                        });
                        sendSms(parseInt(_user.country_code.substring(1, _user.country_code.length)), _user.phone_number, _user.system_lang, msg)
                    }
                }
        ...
        //Translate 알림인 경우
        else if(_user.user_type == global.ROLE.indexOf('translator') + 1 && _request_detail != null) {
            let template_obj: {[k: string]: any} = {};
            template_obj.title = _trans_request?.title;
            template_obj.org_lang = _trans_request?.original_language.name
            template_obj.dst_lang = _request_detail.translate_language.name
            if(_type == 1 && _parent != null && _trans_request != null) {
                if(_trans_request.has_original_video == Trigger.OFF && _parent.has_tc_service == Trigger.ON)
                    template_obj.predict_end_date = (await calc_working_time(_parent.screen_time_limit, _parent.end_time_settings, _trans_request.duration_minute, 5, _trans_request.is_urgent)) * 60 + _trans_request.tc_end_date
                else
                    template_obj.predict_end_date = (await calc_working_time(_parent.screen_time_limit, _parent.end_time_settings, _trans_request.duration_minute, 5, _trans_request.is_urgent)) * 60 + _trans_request.create_date
            }
            template_obj.site_url = config.SITE_URL;
            insertNoticeUser(_user, _notice_ret)

            if(_trans_request != null)
                webPush([_user.player_id], _user.system_lang, convertTranslateMsg(lang[_user.system_lang]["web_push"]["translator_a01"], { title: _trans_request.title, org_lang: _trans_request.original_language.name, dst_lang: _request_detail.translate_language.name }))
                ...
                if(_user.country_code == '+82' && _user.system_lang == 'KO') {
                    ...
                    if(_type == 1 && _trans_request != null && _request_detail != null) {
                        let msg = convertTranslateMsg(lang[_user.system_lang]["sms"]["translator_a01"], {
                            title: _trans_request.title,
                            org_lang: _trans_request.original_language.name,
                            dst_lang: _request_detail.translate_language.name    
                        });
                        sendSms(parseInt(_user.country_code.substring(1, _user.country_code.length)), _user.phone_number, _user.system_lang, msg)
                    }
                }
            ...
            if(_type == 1 && _request_detail != null && _parent != null)
                template_obj.predict_end_date = (await calc_working_time(_parent.screen_time_limit, _parent.end_time_settings, _request_detail.request.duration_minute, 6, _request_detail.request.is_urgent)) * 60 + _request_detail.update_date
            template_obj.site_url = config.SITE_URL;
            insertNoticeUser(_user, _notice_ret)
            if(_trans_request != null)
                webPush([_user.player_id], _user.system_lang, convertTranslateMsg(lang[_user.system_lang]["web_push"]["reviewer_a01"], { title: _trans_request.title, dst_lang: _request_detail.translate_language.name }))
            if(_user.is_sms_notice_on == Trigger.ON) {
                if(_user.country_code == '+82' && _user.system_lang == 'KO') {
                    if(_type == 1)
        ...
        /* --- */
        sendPush(_parent, _admins, _user, _trans_request, _request_detail, _type, _is_read)
    }
    catch(err) {
    }
}
...
export const insertNotice = async(_user: User, _trans_request: TransRequest | null, _request_detail: RequestDetail | null, _type: any, _is_read = Trigger.OFF) => {
    try {
        ...
        if(_parent == undefined)
            _parent = null
        noticeProc(_parent, _admins, _user, _trans_request, _request_detail, _type, _is_read);
    }
    catch(err) {
    }
}

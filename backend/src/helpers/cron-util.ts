    ...
    try {
        ...
        for (let i = 0; i < _result.length; i++) { 
            ...
            for (let j = 0; j < _admins.length; j++) {
                insertNoticeUser(_admins[j], _notice_ret)
                if (_type == 6)
                    webPush([_admins[j].player_id], _admins[j].system_lang, convertTranslateMsg(lang[_admins[i].system_lang]["web_push"]["admin_a04"], { title: _result[i].request.title, org_lang: _result[i].request.original_language.name, dst_lang: _result[i].translate_language.name }))
                else if (_type == 7)
                    ...
                    if (_admins[j].country_code == '+82' && _admins[j].system_lang == 'KO') {
                        ...
                        if (_type == 6)
                            msg = convertTranslateMsg(lang[_admins[j].system_lang]["sms"]["admin_a04"],
                            {
                                ...
                                org_lang: _result[i].request.original_language.name,
                                dst_lang: _result[i].translate_language.name
                            });
                        else if (_type == 7)
...
export const adminA02 = async (_admins: User[]) => {
    try {
        ...
        for (let i = 0; i < _result.length; i++) { 
            ...
            for (let j = 0; j < _admins.length; j++) {
                insertNoticeUser(_admins[j], _notice_ret)
                webPush([_admins[j].player_id], _admins[j].system_lang, convertTranslateMsg(lang[_admins[j].system_lang]["web_push"]["admin_a02"], { title: _result[i].title, org_lang: _result[i].original_language.name }))
            }
            for (let j = 0; j < _admins.length; j++) {
                if (_admins[j].is_sms_notice_on == Trigger.ON) {
                    if (_admins[j].country_code == '+82' && _admins[j].system_lang == 'KO')
                        ...
                        {
                            title: _result[i].title,
                            org_lang: _result[i].original_language.name
                        });
                        sendSms(parseInt(_admins[j].country_code.substring(1, _admins[j].country_code.length)), _admins[j].phone_number, _admins[j].system_lang, msg)        
                    }
                }
...
export const reviewA02A03 = async (_admins:  User[], _type: number) => {
    try {
        ...
        if (_type == 3)
            query = query.andWhere('"request_detail_worker"."predict_end_date" <= ' + cur_time);
        if (_type == 4)
        ...
        for (let i = 0; i < _result.length; i++) {
            ...
            template_obj.dst_lang = _result[i].translate_language.name;
            if(isArray(_result[i].workers))
                template_obj.predict_end_date = _result[i].workers[0].predict_end_date
            template_obj.site_url = config.SITE_URL;    
            let _worker = null;
            if (isArray(_result[i].workers))
            ...
            if (_worker != null) {
                ...
                if(_type == 3)
                    webPush([_worker.player_id], _worker.system_lang, convertTranslateMsg(lang[_worker.system_lang]["web_push"]["reviewer_a02"], { title: _result[i].request.title, dst_lang: _result[i].translate_language.name }))
                else if (_type == 4)
                    webPush([_worker.player_id], _worker.system_lang, convertTranslateMsg(lang[_worker.system_lang]["web_push"]["reviewer_a03"], { title: _result[i].request.title, dst_lang: _result[i].translate_language.name }))
                //sms
...
export const tcA02A03 = async(_admins:  User[], _type: number) => {   
    try {
        ...
        if(_type == 3)
            query = query.andWhere('"trans_request"."tc_predict_end_date" <= ' + cur_time)
        if(_type == 4)
        ...
        for(let i = 0; i < _result.length; i ++) {
            ...
            template_obj.youtube_url = _result[i].youtube_url;
            template_obj.org_lang = _result[i].original_language.name;
            template_obj.predict_end_date = _result[i].tc_predict_end_date;
            template_obj.site_url = config.SITE_URL; 

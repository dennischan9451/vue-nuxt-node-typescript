...
import { insertNotice, youtube_caption_apply } from './../helpers/db-util';
    ...
    static getWorkerRequestList = async(req: Request, res: Response) => {
        try {
            ...
            if(global.ROLE[req.decodedUser.user_type - 1] == 'tc') {
                ...
                THEN request_detail.translate_price_value \
                ELSE company_price.trans_price * trans_request.duration_minute * (100 + ' + user_correction_rate + ') / 100 \
                END as request_detail_temp_price \
                ') 
            }
            else if (global.ROLE[req.decodedUser.user_type - 1] == 'reviewer') {
            ...
            if (global.ROLE[req.decodedUser.user_type - 1] == 'tc') {
                ...
                sum_query = sum_query.addSelect('sum( \
                    CASE \
                    ...
                    ELSE company_price.trans_price * trans_request.duration_minute * (100 + ' + user_correction_rate + ') / 100 \
                    END \
                ) / count(request_detail.id)', 'temp_price_sum')
            }
            else if (global.ROLE[req.decodedUser.user_type - 1] == 'reviewer') {
            ...
            sum_query = sum_query.addSelect('sum(request_detail_worker.price) / count(request_detail.id)', 'work_price_sum')
            ...
            if(isNotEmpty(search)) {
                ...
                !(isNotEmpty(search.end_work_price) && (isNumber(search.end_work_price) || isNumberString(search.end_work_price))) ) {
                 //   query = query.andWhere('"request_detail_worker"."price" >= ' + search.start_work_price);
                    query = query.andWhere(new Brackets(qb => {
                        qb.where('(' + price_field_name1 + ' and "request_detail_worker"."price" >= ' + search.start_work_price + ') or (' + price_field_name2 + ' and ' + price_field_name + ' >= ' + search.start_work_price + ')')
                    }));
                    sum_query = sum_query.andWhere(new Brackets(qb => {
                        qb.where('(' + price_field_name1 + ' and "request_detail_worker"."price" >= ' + search.start_work_price + ') or (' + price_field_name2 + ' and ' + price_field_name + ' >= ' + search.start_work_price + ')')
                    }));
                ...
                (isNotEmpty(search.end_work_price) && (isNumber(search.end_work_price) || isNumberString(search.end_work_price))) ) {
                    query = query.andWhere(new Brackets(qb => {
                        qb.where('(' + price_field_name1 + ' and "request_detail_worker"."price" <= ' + search.end_work_price + ') or (' + price_field_name2 + ' and ' + price_field_name + ' <= ' + search.end_work_price + ')')
                    }));
                    sum_query = sum_query.andWhere(new Brackets(qb => {
                        qb.where('(' + price_field_name1 + ' and "request_detail_worker"."price" <= ' + search.end_work_price + ') or (' + price_field_name2 + ' and ' + price_field_name + ' <= ' + search.end_work_price + ')')
                    }));
                ...
                (isNotEmpty(search.end_work_price) && (isNumber(search.end_work_price) || isNumberString(search.end_work_price))) ) {
                    query = query.andWhere(new Brackets(qb => {
                        qb.where('(' + price_field_name1 + ' and "request_detail_worker"."price" >= ' + search.start_work_price + ' and "request_detail_worker"."price" <= ' + search.end_work_price + ') \
            ...
            return res.json({ errorCode: 0, errorMsg: "", data: {
                ...
                work_price_sum: (isEmpty(_result_sum.work_price_sum1) ? 0 : parseInt(_result_sum.work_price_sum1)) + (isEmpty(_result_sum.work_price_sum2) ? 0 : parseInt(_result_sum.work_price_sum2))
            } });
        }
        catch(error) {
            console.log(error)

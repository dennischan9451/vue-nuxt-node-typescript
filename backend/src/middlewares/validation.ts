...
export const updateMyInfoValidation = (req: Request, res: Response, next: NextFunction) => {
    ...
    if( (global.ROLE[req.decodedUser.user_type-1] != 'manager' && global.ROLE[req.decodedUser.user_type-1] != 'requester' && global.ROLE[req.decodedUser.user_type-1] != 'company' && (isEmpty(user_name) || isEmpty(country_code) || isEmpty(phone_number) || isEmpty(email) || isEmpty(system_lang)
